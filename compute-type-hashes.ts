 ...
 *
 * Run: npx ts-node compute-type-hashes.ts
 */
 ...
 */
function encodeType(name: string, fields: Array<{ name: string; type: string }>): string {
 ...
 * Compute type hash
 */
function computeTypeHash(name: string, fields: Array<{ name: string; type: string }>): string {
  const typeString = encodeType(name, fields);
  const hash = keccak256(toUtf8Bytes(typeString));
  return hash;
}
